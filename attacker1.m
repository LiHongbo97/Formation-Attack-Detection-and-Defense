function [ output_args ] = attacker1( input_args )
...
%     ob_temp=ob_temp';
    ...
    for count=1:countmax
        k=k+1;
        if(k>100&&rem(k,20)==1)
            if abs(attmse(N+1,(k-6)/5)-attmse(N+1,(k-36)/5))<0.03
                q=rem(q+1,5)+1
            end
        end
%         %%%做直线
...
%                 pose_y_attpurpose(k+1)=(ideal_posey(1,(k-1)/5)+ideal_posey(2,(k-1)/5))/2+1;
                pose_x_attpurpose(k+1)=ideal_posex(4,(k-1)/5)+1;
                pose_y_attpurpose(k+1)=ideal_posey(4,(k-1)/5)+1;
            end
        elseif(k==1)
            pose_x_attpurpose(k+1)=sum_pose_x/N;
            pose_y_attpurpose(k+1)=sum_pose_y/N;
...
%     xlswrite('attmse.xlsx',attmse);%写入xlsx文件进行外部归一化
    ...
    hold on
    plot(pose_x_att(1),pose_y_att(1),'*','color',color(1,N+1),'LineWidth',1);
    hold on
    for i=1:N-1
    ...
    hold on
    plot(pose_x_att(k),pose_y_att(k),'o','color',color(1,N+1),'LineWidth',2);
    hold on
    plot(ob_temp(:,1),ob_temp(:,2),'Xk','LineWidth',2);hold on;
    grid on;
    xlabel('x');
